...
class PlayerAI(BaseAI):
    ...
    def getMove(self, grid):
        return self.max(grid, Depth, -inf, inf)[0]
    ...
    def max(self, grid, depth, alpha, beta):
        ...
        for m in moves:
            ...
            temp_grid.move(m)
            score = self.expect(temp_grid, depth - 1, bestScore, beta)
            if score > bestScore:
                bestScore = score
                bestMove = m
    ...
    def expect(self, grid, depth, alpha, beta):
        if depth == 0 or not grid.canMove():
            # print self.heuristic(grid)
        ...
        for c in cells:
            ...
            score = self.max(temp_grid, depth - 1, alpha, bestScore2)[1]
            if score < bestScore2:
                bestScore2 = score
        ...
        for c in cells:
            ...
            score = self.max(temp_grid, depth - 1, alpha, bestScore4)[1]
            if score < bestScore4:
                bestScore4 = score
            # print str(depth) + "bestScoreMin: " + str(bestScore)
            if bestScore4 < alpha:
